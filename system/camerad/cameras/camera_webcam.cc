...
//                    .bayer_flip = false,
    ...
    void run_camera(CameraState *s, cv::VideoCapture &video_cap, float *ts) {
        ...
        while (!do_exit) {
            ...
            s->buf.camera_bufs_metadata[buf_idx].timestamp_sof = nanos_since_boot();
...
void cameras_init(VisionIpcServer *v, MultiCameraState *s, cl_device_id device_id, cl_context ctx) {
    camera_init(v, &s->road_cam, CAMERA_ID_LGC920, 20, device_id, ctx, VISION_STREAM_ROAD);
//    camera_init(v, &s->road_cam, CAMERA_ID_LGC920, 20, device_id, ctx, VISION_STREAM_WIDE_ROAD);

    camera_init(v, &s->driver_cam, CAMERA_ID_LGC615, 10, device_id, ctx, VISION_STREAM_DRIVER);
